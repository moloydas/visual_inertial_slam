...
# Kalman Filter
class kalman_filter:
    def __init__(self, vx=0.1, vz=50) -> None:
        self.vz = vz
        self.W = np.eye(6)
        self.W[:3,:3] *= 0.1
        self.W[3:,3:] *= 0.1
    ...
    def update_state(self, my_robot, pose, sigma, i_T_c, map_pts, measurements, n_landmarks, update_idx):
        nt = measurements.shape[1]
        HR = np.zeros((4*nt, 6))
        ...
        q = c_T_i @ inversePose(pose) @ norm_map
        pred_z = my_robot.Ks @ projection(q.T).T
        zt = measurements.flatten(order="F")
        ...
        for j in range(len(update_idx)):
            ...
            q = c_T_i @ i_T_w @ m_j
            d_pi = np.squeeze(projectionJacobian(q.T))
            HM[4*i:(4*i+4), 3*j_idx:(3*j_idx+3)] = my_robot.Ks @ d_pi @ c_T_i @ i_T_w @ P.T
            i += 1
        ...
        for j in range(len(update_idx)):
            ...
            q = c_T_i @ inversePose(pose) @ m_j
            d_pi = np.squeeze(projectionJacobian(q.T))
            HR[4*j:(4*j+4), :] = -my_robot.Ks @ d_pi @ c_T_i @ circle_dot(pr3_utils.inversePose(pose) @ m_j)
        ...
        innovation = zt - pred_z[:, update_idx].flatten(order="F")
        # print("innovation: ", np.linalg.norm(innovation))
        KG_pose = KG[:6, :]
        KG_map = KG[6:, :]
        ...
        pose = pose @ pr3_utils.axangle2pose(np.expand_dims(del_mu_pose, axis=0))
        map_pts = map_pts + del_mu_map.reshape(-1,1)

        sigma = (np.eye(3*n_landmarks + 6) - KG @ H) @ sigma

        return pose,  map_pts, sigma #pose_sigma, m_sigma
